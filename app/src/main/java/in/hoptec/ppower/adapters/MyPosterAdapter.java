...
import in.hoptec.ppower.MyVideos;
import in.hoptec.ppower.R;
import in.hoptec.ppower.database.Feed;
import in.hoptec.ppower.utl;


public class MyPosterAdapter extends RecyclerView.Adapter<MyPosterAdapter.CustomViewHolder> {
    public List<Feed> feedItemList;
    private Context ctx;

    public MyPosterAdapter(Context context, List<Feed> feedItemList, CallBacks cab) {
...
//        YoYo.with(Techniques.SlideInLeft).duration(500).playOn( view);
    ...
    public void onBindViewHolder(final CustomViewHolder cv, final int i) {
      ...
      final Feed cat=feedItemList.get(i);
        ...
        cv.like.setImageResource(R.drawable.x);
    ...
    {
        return MyVideos.likes.contains(":"+ id);
    }
    int qan;
    @Override
